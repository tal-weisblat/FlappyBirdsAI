...
def mutateBird(bird_list, BIRDS_NUM):
    ...
    for i in range(BIRDS_NUM):
        ...
        for j in range(4):
            list_rand = np.random.normal(0,STD,4) 
            list_rand = list_rand.tolist()
            child_net.linear1.weight.data[j] = parent_net.linear1.weight.data[j] + torch.tensor([list_rand])
        # weights-2 (4 weights)
        list_rand = np.random.normal(0,STD,4) 
        list_rand = list_rand.tolist()
        ...
        # bias-2 
        list_rand = np.random.normal(0,STD,1)
        list_rand = list_rand.tolist()
